...
;;; locations
;;; Check http://www.latlong.net/place/san-francisco-ca-usa-594.html

(defrecord Location [name 
                     lat ;; latitude
                     lng ;; longitude
                     uri ;; Main web page
                     tags ;; simple tags in form of keywords that marks location, e.g. :climbing, :city, :national-park
...
(def los-angeles-airport (make-location "Los Angeles Airport" 33.942791 -118.410042 "" [:city]))
(def san-francisco (make-location "San Francisco" 37.773972 -122.431297 "" [:city]))
(def salt-lake-city (make-location "Salt Lake City" 40.758701 -111.876183 "" [:city]))
...
(defn map-component-did-mount [this]
  (let [my-map (create-map salt-lake-city (reagent/dom-node this))]
